<?php
/**
 * Spanish (ES) translations for SDS sections.
 */
return [
    'section1' => [
        'title'           => 'Identificación',
        'recommended_use' => 'Tinta de impresión para aplicaciones comerciales e industriales.',
        'restrictions'    => 'Solo para uso profesional/industrial. No para uso doméstico.',
    ],
    'section2' => [
        'title'         => 'Identificación de Peligros',
        'other_hazards' => 'Ninguno conocido.',
    ],
    'section3' => [
        'title'            => 'Composición / Información sobre los Ingredientes',
        'trade_secret_note' => 'La identidad química específica y/o el porcentaje exacto de composición ha sido retenido como secreto comercial de acuerdo con 29 CFR 1910.1200(i).',
    ],
    'section4' => [
        'title'      => 'Medidas de Primeros Auxilios',
        'inhalation' => 'Trasladar a aire fresco. Si persiste la dificultad respiratoria, buscar atención médica.',
        'skin'       => 'Quitar la ropa contaminada. Lavar la piel con agua y jabón. Si la irritación persiste, buscar atención médica.',
        'eyes'       => 'Enjuagar los ojos con abundante agua durante al menos 15 minutos. Buscar atención médica si la irritación persiste.',
        'ingestion'  => 'No provocar el vómito. Enjuagar la boca con agua. Buscar atención médica si se desarrollan síntomas.',
        'notes'      => 'Tratar sintomáticamente. Mostrar esta HDS al personal médico.',
        'inhalation_fatal'     => 'Trasladar INMEDIATAMENTE a la persona a aire fresco. Si no respira, administrar respiración artificial. Llamar inmediatamente a un centro de toxicología o a un médico.',
        'inhalation_toxic'     => 'Trasladar inmediatamente a aire fresco. Si tiene dificultad para respirar, administrar oxígeno. Buscar atención médica inmediata.',
        'skin_corrosive'       => 'Enjuagar inmediatamente la piel con abundante agua durante al menos 20 minutos. Quitar toda la ropa y calzado contaminados. Buscar atención médica inmediata. Lavar la ropa contaminada antes de reutilizarla.',
        'skin_sensitizer'      => 'Quitar la ropa contaminada. Lavar la piel con agua y jabón. Si se desarrolla una reacción alérgica, buscar atención médica. Evitar exposición futura.',
        'eyes_corrosive'       => 'Enjuagar inmediatamente los ojos con abundante agua durante al menos 30 minutos. Buscar atención médica inmediata. No permitir que la víctima se frote los ojos.',
        'eyes_serious_damage'  => 'Enjuagar inmediatamente los ojos con abundante agua durante al menos 20 minutos. Buscar atención médica inmediata.',
        'ingestion_aspiration' => 'NO provocar el vómito — peligro de aspiración. Si se produce vómito espontáneo, mantener la cabeza por debajo de las caderas. Buscar atención médica inmediata.',
        'ingestion_toxic'      => 'Enjuagar la boca con agua. No provocar el vómito a menos que lo indique el personal médico. Llamar inmediatamente a un centro de toxicología o a un médico.',
    ],
    'section5' => [
        'title'            => 'Medidas de Lucha contra Incendios',
        'suitable_media'   => 'Pulverización de agua, polvo químico seco, dióxido de carbono (CO2), espuma.',
        'unsuitable_media' => 'No usar chorro directo de agua ya que puede propagar el fuego.',
        'specific_hazards' => 'La combustión puede producir monóxido de carbono, dióxido de carbono y otros gases tóxicos.',
        'firefighter_advice' => 'Usar equipo de respiración autónomo (ERA) y equipo de protección completo.',
        'unsuitable_water_reactive' => 'NO usar agua. El producto reacciona con agua. Usar polvo químico seco, arena seca o dióxido de carbono (CO2).',
        'suitable_oxidizer'         => 'Pulverización de agua (grandes cantidades), espuma. No usar polvo químico seco en incendios grandes con oxidantes.',
        'specific_hazards_oxidizer' => 'Oxidante — puede intensificar el fuego. La combustión puede producir monóxido de carbono y otros gases tóxicos.',
        'specific_hazards_organic_peroxide' => 'Puede incendiarse o explotar al calentarse. La combustión puede producir monóxido de carbono y otros gases tóxicos.',
        'firefighter_advice_explosive' => 'Evacuar el área. Combatir el fuego desde una posición protegida. Usar ERA y equipo de protección completo.',
        'flash_point_low_warning'   => 'Líquido y vapores altamente inflamables. Mantener alejado del calor, chispas, llamas abiertas y superficies calientes.',
    ],
    'section6' => [
        'title'                => 'Medidas en Caso de Derrame Accidental',
        'personal_precautions' => 'Usar EPP apropiado (ver Sección 8). Evitar el contacto con la piel y los ojos.',
        'environmental'        => 'Evitar que entre en desagües, alcantarillas y vías fluviales.',
        'containment'          => 'Contener el derrame con material absorbente inerte. Recoger en recipientes adecuados para su eliminación.',
        'precautions_corrosive'    => 'Usar EPP apropiado incluyendo traje resistente a químicos, guantes y protector facial (ver Sección 8). Evitar todo contacto con la piel y los ojos. Evacuar al personal no protegido.',
        'precautions_acute_toxic'  => 'Usar EPP apropiado incluyendo equipo de respiración autónomo (ERA) (ver Sección 8). Evacuar al personal no protegido del área afectada.',
        'environmental_aquatic'    => 'Evitar que entre en desagües, alcantarillas y vías fluviales. Tóxico para la vida acuática. Notificar a las autoridades si el producto entra en vías fluviales.',
        'containment_liquid'       => 'Detener la fuga si es seguro hacerlo. Represar o contener el derrame. Absorber con material absorbente inerte. Recoger en recipientes adecuados para su eliminación.',
        'containment_solid'        => 'Evitar la generación de polvo. Barrer o aspirar el material. Recoger en recipientes adecuados para su eliminación.',
    ],
    'section7' => [
        'title'    => 'Manipulación y Almacenamiento',
        'handling' => 'Usar en áreas bien ventiladas. Evitar el contacto con la piel y los ojos. Usar EPP apropiado.',
        'storage'  => 'Almacenar en un lugar fresco, seco y bien ventilado. Mantener los recipientes cerrados.',
        'handling_flammable'       => 'Usar en áreas bien ventiladas. Mantener alejado del calor, chispas, llamas abiertas y superficies calientes. No fumar. Usar herramientas anti-chispas. Tomar medidas contra descargas electrostáticas.',
        'handling_oxidizer'        => 'Mantener alejado de materiales combustibles, calor y fuentes de ignición. No mezclar con materiales inflamables o combustibles.',
        'handling_water_reactive'  => 'Mantener seco. No manipular en condiciones húmedas. Usar EPP apropiado.',
        'handling_pyrophoric'      => 'Manipular bajo atmósfera de gas inerte. Proteger de la humedad y el aire. Usar EPP apropiado.',
        'handling_self_reactive'   => 'Mantener alejado del calor. Manipular y abrir el recipiente con cuidado.',
        'storage_flammable'        => 'Almacenar en un lugar fresco, seco y bien ventilado, alejado del calor y fuentes de ignición. Mantener los recipientes cerrados. Conectar a tierra los recipientes durante la transferencia.',
        'storage_oxidizer'         => 'Almacenar en un lugar fresco, seco y bien ventilado. Mantener separado de materiales combustibles, agentes reductores y sustancias inflamables.',
        'storage_water_reactive'   => 'Almacenar en un lugar fresco y seco. Proteger de la humedad y el agua. Mantener los recipientes herméticamente cerrados.',
        'storage_pyrophoric'       => 'Almacenar bajo gas inerte. Proteger del aire y la humedad. Mantener los recipientes herméticamente cerrados.',
        'storage_self_heating'     => 'Almacenar en un lugar fresco. Mantener alejado de fuentes de calor. Monitorear la temperatura de almacenamiento.',
    ],
    'section8' => [
        'title'           => 'Controles de Exposición / Protección Personal',
        'engineering'     => 'Usar ventilación local o controles de ingeniería para mantener las concentraciones por debajo de los límites.',
        'respiratory'     => 'Usar respirador aprobado por NIOSH si se exceden los límites de exposición.',
        'hand_protection' => 'Guantes resistentes a productos químicos (nitrilo o neopreno).',
        'eye_protection'  => 'Gafas de seguridad con protección lateral.',
        'skin_protection' => 'Usar ropa protectora para prevenir el contacto con la piel.',
    ],
    'section9'  => ['title' => 'Propiedades Físicas y Químicas'],
    'section10' => [
        'title'            => 'Estabilidad y Reactividad',
        'reactivity'       => 'Sin reacción peligrosa conocida bajo condiciones normales de uso.',
        'stability'        => 'Estable bajo las condiciones de almacenamiento recomendadas.',
        'conditions_avoid' => 'Calor excesivo, chispas, llamas abiertas, oxidantes fuertes.',
        'incompatible'     => 'Agentes oxidantes fuertes, ácidos fuertes, bases fuertes.',
        'decomposition'    => 'Monóxido de carbono, dióxido de carbono y otros gases tóxicos.',
        'conditions_avoid_water_reactive' => 'Agua, humedad, calor excesivo, chispas, llamas abiertas.',
        'conditions_avoid_pyrophoric'     => 'Aire, humedad, calor excesivo.',
        'conditions_avoid_self_reactive'  => 'Calor, fricción, impacto, contaminación. Evitar temperaturas por encima de los límites de almacenamiento recomendados.',
        'incompatible_oxidizer'           => 'Materiales combustibles, agentes reductores, materiales orgánicos, polvos metálicos.',
        'incompatible_water_reactive'     => 'Agua, humedad, ácidos fuertes, bases fuertes.',
        'incompatible_flammable'          => 'Agentes oxidantes fuertes, ácidos fuertes, bases fuertes, halógenos.',
        'incompatible_pyrophoric'         => 'Aire, humedad, agua, agentes oxidantes.',
        'decomposition_nitrogen'          => 'Monóxido de carbono, dióxido de carbono, óxidos de nitrógeno y otros gases tóxicos.',
        'decomposition_sulfur'            => 'Monóxido de carbono, dióxido de carbono, óxidos de azufre y otros gases tóxicos.',
        'decomposition_halogen'           => 'Monóxido de carbono, dióxido de carbono, haluros de hidrógeno y otros gases tóxicos.',
    ],
    'section11' => [
        'title'           => 'Información Toxicológica',
        'acute_toxicity'  => 'Basado en los datos disponibles, no se cumplen los criterios de clasificación.',
        'chronic_effects' => 'La exposición prolongada o repetida puede causar sequedad o agrietamiento de la piel.',
        'carcinogenicity' => 'Ningún componente está listado como cancerígeno por IARC, NTP u OSHA.',
    ],
    'section12' => [
        'title'       => 'Información Ecológica',
        'ecotoxicity' => 'No hay datos disponibles sobre la mezcla.',
        'persistence' => 'No hay datos disponibles.',
        'bioaccumulation' => 'No hay datos disponibles.',
        'note' => 'Esta sección no es requerida por OSHA HazCom pero se incluye según las directrices del GHS.',
        'ecotoxicity_acute'        => 'Tóxico para los organismos acuáticos según la clasificación de peligros.',
        'ecotoxicity_chronic'      => 'Tóxico para los organismos acuáticos, con efectos nocivos duraderos, según la clasificación de peligros.',
        'ecotoxicity_acute_chronic' => 'Tóxico para los organismos acuáticos, con efectos nocivos duraderos, según la clasificación de peligros.',
        'environmental_warning'    => 'Evitar la liberación al medio ambiente. Prevenir la entrada en cursos de agua, alcantarillas y suelos.',
    ],
    'section13' => [
        'title'   => 'Consideraciones de Eliminación',
        'methods' => 'Eliminar de acuerdo con todas las regulaciones aplicables.',
        'note'    => 'Esta sección no es requerida por OSHA HazCom pero se incluye según las directrices del GHS.',
        'methods_ignitable'   => 'Eliminar de acuerdo con todas las regulaciones aplicables. Este producto puede clasificarse como residuo peligroso inflamable (EPA D001) debido a su punto de inflamación. No verter en alcantarillas, desagües o vías fluviales.',
        'methods_corrosive'   => 'Eliminar de acuerdo con todas las regulaciones aplicables. Este producto puede clasificarse como residuo peligroso corrosivo (EPA D002). No verter en alcantarillas, desagües o vías fluviales.',
        'methods_toxic'       => 'Eliminar de acuerdo con todas las regulaciones aplicables. Este producto puede contener componentes tóxicos sujetos a regulaciones de residuos peligrosos. No verter en alcantarillas, desagües o vías fluviales.',
        'methods_reactive'    => 'Eliminar de acuerdo con todas las regulaciones aplicables. Este producto puede clasificarse como residuo peligroso reactivo (EPA D003). No verter en alcantarillas, desagües o vías fluviales.',
        'methods_aquatic'     => 'Eliminar de acuerdo con todas las regulaciones aplicables. No permitir que el producto llegue a vías fluviales — tóxico para la vida acuática. No verter en alcantarillas, desagües o vías fluviales.',
    ],
    'section14' => [
        'title' => 'Información de Transporte',
        'note'  => 'Verificar la clasificación con el transportista antes del envío.',
    ],
    'section15' => [
        'title'       => 'Información Reglamentaria',
        'osha_status' => 'Este producto está clasificado como peligroso bajo OSHA HazCom 2012.',
        'tsca_status' => 'Todos los componentes están listados o exentos del inventario TSCA.',
        'note'        => 'Esta sección se incluye según las directrices del GHS.',
    ],
    'section16' => [
        'title'         => 'Otra Información',
        'disclaimer'    => 'La información proporcionada en esta Hoja de Datos de Seguridad es correcta según nuestro mejor conocimiento. Debe considerarse como una guía para el manejo seguro.',
        'abbreviations' => 'CAS = Chemical Abstracts Service; GHS = Sistema Globalmente Armonizado; OSHA = Administración de Seguridad y Salud Ocupacional; VOC = Compuestos Orgánicos Volátiles.',
    ],

    // Document-level strings (header, footer, section banner)
    'document' => [
        'title'           => 'HOJA DE DATOS DE SEGURIDAD',
        'section_prefix'  => 'SECCIÓN',
        'page'            => 'Página',
        'page_of'         => 'de',
        'revision_prefix' => 'Rev.',
    ],

    'labels' => [
        // Section 1
        'product_identifier'    => 'Identificador del Producto',
        'product_family'        => 'Familia del Producto',
        'recommended_use'       => 'Uso Recomendado',
        'restrictions'          => 'Restricciones de Uso',
        'manufacturer_info'     => 'Información del Fabricante / Proveedor',
        'company'               => 'Empresa',
        'address'               => 'Dirección',
        'phone'                 => 'Teléfono',
        'emergency'             => 'Emergencia',

        // Section 2
        'pictograms'            => 'Pictogramas',
        'ghs_classification'    => 'Clasificación GHS',
        'physical_hazards'      => 'Peligros Físicos',
        'health_hazards'        => 'Peligros para la Salud',
        'environmental_hazards' => 'Peligros Ambientales',
        'hazard_statements'     => 'Indicaciones de Peligro',
        'precautionary_statements' => 'Consejos de Prudencia',
        'ppe_recommendations'   => 'Equipo de Protección Personal (EPP) Recomendado',
        'other_hazards'         => 'Otros Peligros',
        'ppe_wear_eye'          => 'Usar Protección Ocular',
        'ppe_wear_gloves'       => 'Usar Guantes',
        'ppe_wear_respiratory'  => 'Usar Protección Respiratoria',
        'ppe_wear_skin'         => 'Usar Ropa Protectora',

        // Section 3
        'type'                  => 'Tipo',
        'cas_number'            => 'Número CAS',
        'chemical_name'         => 'Nombre Químico',
        'concentration'         => 'Concentración',
        'hazardous_only_note'   => 'Solo se enumeran los ingredientes peligrosos. Los componentes no peligrosos se omiten.',
        'no_hazardous_note'     => 'No hay ingredientes peligrosos por encima de los umbrales de divulgación.',
        'mixture'               => 'Mezcla',

        // Section 4 (First-Aid)
        'inhalation'            => 'Inhalación',
        'skin_contact'          => 'Contacto con la Piel',
        'eye_contact'           => 'Contacto con los Ojos',
        'ingestion'             => 'Ingestión',
        'notes_to_physician'    => 'Notas para el Médico',

        // Section 5 (Fire-Fighting)
        'suitable_media'        => 'Medios de Extinción Adecuados',
        'unsuitable_media'      => 'Medios de Extinción No Adecuados',
        'specific_hazards'      => 'Peligros Específicos',
        'firefighter_advice'    => 'Consejo para Bomberos',

        // Section 6 (Accidental Release)
        'personal_precautions'  => 'Precauciones Personales',
        'environmental_precautions' => 'Precauciones Ambientales',
        'containment_cleanup'   => 'Contención y Limpieza',

        // Section 7 (Handling and Storage)
        'handling'              => 'Manipulación',
        'storage'               => 'Almacenamiento',

        // Section 8 (Exposure Controls) — table headers
        'engineering_controls'  => 'Controles de Ingeniería',
        'respiratory_protection' => 'Protección Respiratoria',
        'hand_protection'       => 'Protección de las Manos',
        'eye_protection'        => 'Protección de los Ojos',
        'skin_protection'       => 'Protección de la Piel',
        'respiratory'           => 'Respiratoria',
        'skin_body'             => 'Protección de la Piel/Cuerpo',
        'el_cas'                => 'CAS',
        'el_chemical'           => 'Químico',
        'el_type'               => 'Tipo',
        'el_value'              => 'Valor',
        'el_units'              => 'Unidades',
        'el_conc_pct'           => 'Conc%',
        'el_notes'              => 'Notas',

        // Section 9 (Physical/Chemical Properties)
        'physical_state'        => 'Estado Físico',
        'color'                 => 'Color',
        'appearance'            => 'Apariencia',
        'odor'                  => 'Olor',
        'boiling_point'         => 'Punto de Ebullición',
        'flash_point'           => 'Punto de Inflamación',
        'solubility'            => 'Solubilidad',
        'specific_gravity'      => 'Gravedad Específica',
        'voc_lb_gal'            => 'COV (lb/gal) (EPA Method 24)',
        'voc_less_we'           => 'COV menos A&E (lb/gal)',
        'voc_wt_pct'            => 'COV (% en peso)',
        'solids_wt_pct'         => 'Sólidos (% en peso)',
        'solids_vol_pct'        => 'Sólidos (% en vol)',

        // Section 10 (Stability and Reactivity)
        'reactivity'            => 'Reactividad',
        'chemical_stability'    => 'Estabilidad Química',
        'conditions_avoid'      => 'Condiciones a Evitar',
        'incompatible_materials' => 'Materiales Incompatibles',
        'decomposition_products' => 'Productos de Descomposición Peligrosos',

        // Section 11 (Toxicological Information)
        'acute_toxicity'        => 'Toxicidad Aguda',
        'chronic_effects'       => 'Efectos Crónicos',
        'carcinogenicity'       => 'Carcinogenicidad',
        'component_tox_data'    => 'Datos Toxicológicos por Componente',
        'health_hazard'         => 'Peligro para la Salud',

        // Section 12 (Ecological Information)
        'ecotoxicity'           => 'Ecotoxicidad',
        'persistence'           => 'Persistencia y Degradabilidad',
        'bioaccumulation'       => 'Potencial de Bioacumulación',

        // Section 13 (Disposal)
        'disposal_methods'      => 'Métodos de Eliminación',

        // Section 14 (Transport)
        'un_number'             => 'Número ONU',
        'proper_shipping_name'  => 'Denominación Oficial de Transporte',
        'transport_hazard_class' => 'Clase de Peligro',
        'packing_group'         => 'Grupo de Embalaje',

        // Section 15 (Regulatory)
        'osha_status'           => 'Estado OSHA',
        'tsca_status'           => 'Estado TSCA',
        'sara_313_title'        => 'SARA 313 / Informe TRI',
        'hap_title'             => 'Ley de Aire Limpio Sección 112(b) — Contaminantes Peligrosos del Aire (HAP)',
        'hap_triggering'        => 'Químico HAP Desencadenante',
        'hap_wt_pct'            => '% en Peso en Fórmula',
        'hap_total'             => 'Contenido Total de HAP',
        'hap_none'              => 'Este producto no contiene ningún HAP de la EPA listado bajo la Sección 112(b) de la Ley de Aire Limpio.',
        'prop65_title'          => 'Proposición 65 de California',
        'prop65_none'           => 'No se sabe que este producto contenga sustancias químicas listadas bajo la Proposición 65 de California.',
        'snur_title'            => 'Reglas de Nuevo Uso Significativo de la EPA (SNUR)',
        'state_regulations'     => 'Regulaciones Estatales',

        // Section 16 (Other Information)
        'revision_date'         => 'Fecha de Revisión',
        'abbreviations'         => 'Abreviaturas',
        'disclaimer'            => 'DESCARGO DE RESPONSABILIDAD',

        // Generic / shared
        'not_determined'        => 'No determinado',
        'not_regulated'         => 'No regulado',
        'not_applicable'        => 'No aplica',
        'note'                  => 'Nota',
        'revision_note'         => 'Nota de Revisión',
        'uv_acrylate_note'      => 'Información de Acrilato UV',
    ],
];
