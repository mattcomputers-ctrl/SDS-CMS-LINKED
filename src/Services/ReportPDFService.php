...
class ReportPDFService
{
    ...
    /** @var string Configurable disclaimer text. */
     ...
     * @param  string $disclaimer  Optional disclaimer text (from admin settings)
     * @return string              Raw PDF bytes
     */
    public function generate(array $reportData, string $disclaimer = ''): string
    {
    ...
    private function renderShippingTable(\TCPDF $pdf, array $data): void
    {
        ...
        // Column widths — must sum to usable page width (279 - 15 - 15 = 249 for landscape letter)
        $w = [24, 30, 95, 24, 18, 18, 20, 20];
        ...
        foreach ($data['lines'] as $line) {
            ...
            $pdf->Cell($w[1], 5, $this->truncate($line['item_code'], 20), 1, 0, 'L', $fill);
            $pdf->Cell($w[2], 5, $this->truncate($line['description'], 65), 1, 0, 'L', $fill);
