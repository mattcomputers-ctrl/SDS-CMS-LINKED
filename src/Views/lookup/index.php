            ...
            <?php foreach ($items as $r): ?>
                <tr>
                    ...
                    <td><?php if ($r['has_es'] && !empty($r['sds_id_es'])): ?>
                        <a href="/lookup/download/<?= (int) $r['sds_id_es'] ?>" class="badge badge-success pdf-link" title=" Spanish SDS v<?= (int) $r['ver_es'] ?>">PDF</a>
                    <?php else: ?>—<?php endif; ?></td>
                    <td><?php if ($r['has_fr'] && !empty($r['sds_id_fr'])): ?>
                        <a href="/lookup/download/<?= (int) $r['sds_id_fr'] ?>" class="badge badge-success pdf-link" title=" French SDS v<?= (int) $r['ver_fr'] ?>">PDF</a>
                    <?php else: ?>—<?php endif; ?></td>
                    <td><?php if ($r['has_de'] && !empty($r['sds_id_de'])): ?>
                        <a href="/lookup/download/<?= (int) $r['sds_id_de'] ?>" class="badge badge-success pdf-link" title=" German SDS v<?= (int) $r['ver_de'] ?>">PDF</a>
                    <?php else: ?>—<?php endif; ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
